export interface DshTestkitSkillDefinition {
  readonly name: 'dsh-testkit'
...
const description = 'Use when creating, modifying, reviewing, testing, or releasing a DeepSeek Harness plugin, or diagnosing install, boot, register, update, uninstall, reboot, cleanup, residue, or flaky lifecycle behavior. Configure and run DSH Testkit against a real host, then interpret and retain its evidence.'

const content = `# DSH Testkit Lifecycle Testing
...
export const DSH_TESTKIT_SKILL: DshTestkitSkillDefinition = Object.freeze({
  name: 'dsh-testkit',
  description,
  content,
...
export function renderDshTestkitSkillFile(): string {
  return [
    '---',
    `name: ${DSH_TESTKIT_SKILL.name}`,
    `description: ${JSON.stringify(DSH_TESTKIT_SKILL.description)}`,
    '---',
    '',
    DSH_TESTKIT_SKILL.content,
  ].join('\n')
}
